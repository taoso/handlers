...
func buildCommonLogLine(req *http.Request, url url.URL, ts time.Time, status int, size int, vhost bool) []byte {
	...
	if req.ProtoMajor > 1 && req.Method == "CONNECT" {
		uri = req.Host
		if vhost {
			vh = "-"
		} else {
	...
	buf := make([]byte, 0, 3*(len(vh)+len(host)+len(username)+len(req.Method)+len(uri)+len(req.Proto)+50)/2)
	if vhost {
		buf = append(buf, vh...)
		buf = append(buf, ' ')
	}
...
// CombinedVLoggingHandler return a http.Handler that wraps h and logs requests to out in
